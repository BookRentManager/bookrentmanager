-- Update the update_booking_on_payment trigger to pass booking_update_type
-- This prevents duplicate emails and provides proper context for email content
...
BEGIN
  ...
  IF NEW.payment_link_status = 'paid'::payment_link_status OR (NEW.paid_at IS NOT NULL AND (OLD.paid_at IS NULL OR OLD IS NULL)) THEN
    ...
    IF v_booking.status = 'draft'::booking_status AND v_total_paid >= v_required_down_payment THEN
      ...
      -- Pass booking_update_type to differentiate initial confirmation from additional payments
      BEGIN
