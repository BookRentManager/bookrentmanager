-- Update the trigger function to only send email for first payment
...
BEGIN
  ...
  IF NEW.payment_link_status = 'paid'::payment_link_status OR (NEW.paid_at IS NOT NULL AND (OLD.paid_at IS NULL OR OLD IS NULL)) THEN
    ...
    -- CRITICAL FIX: Mark email as sent IMMEDIATELY to prevent race conditions
    -- Do this BEFORE any async operations
    UPDATE payments
    SET confirmation_email_sent_at = NOW()
    WHERE id = NEW.id AND confirmation_email_sent_at IS NULL;
    
    -- If the update affected 0 rows, it means another process already marked it
    -- In that case, exit early to prevent duplicate processing
    IF NOT FOUND THEN
      RETURN NEW;
    END IF;
    ...
    -- ONLY THIS SCENARIO SENDS EMAIL
    ...
    -- Always update client invoice payment status for any payment
    -- (regardless of whether email was sent)
    UPDATE client_invoices
    SET payment_status = CASE 
      WHEN v_total_paid >= v_booking.amount_total THEN 'paid'
      ELSE 'partial'
